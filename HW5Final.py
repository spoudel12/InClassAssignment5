...
#WRITE YOUR CODE HERE FOR THE RECURSIVE SORTING FUNCTION
    ...
    return quicksort(lessThan) + [pivot] + quicksort(geaterThan)
...
# WRITE YOUR MAIN FUNCTION HERE TO READ IN YOUR numbers.txt FILE, RUN THE LIST THROUGH YOUR SORTING ALGORITHM,
    ...
    data = patn.split(',') #Line 113 and 115 used to take out brackets found online

    orignialList = [int(item) for item in data]
    ...
    #print the sorted list in the output text file

    #with open(r'sorted1.txt', 'w') as sp1:

    with open("sorted.txt", "w") as sortedFile:

        sortedFile.write(str(sortedList))
